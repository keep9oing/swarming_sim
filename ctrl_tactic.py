...
def commands(states_q, states_p, obstacles, walls, r, d, r_prime, d_prime, targets, targets_v, targets_enc, targets_v_enc, swarm_prox, tactic_type, centroid, escort):   
    ...
    for k_node in range(states_q.shape[1]): 
        ...
        if tactic_type == 1:
            ...
            #u_int[:,k_node] = saber_tools.compute_cmd_a(states_q[:,k_node].reshape((3,1)), states_p[:,k_node].reshape((3,1)), targets[:,k_node].reshape((3,1)), targets_v[:,k_node].reshape((3,1)), k_node, r, d, r_prime, d_prime)    
            #print('test')
        ...
        elif tactic_type == 2 or tactic_type == 8:
